...
/* Lin Geny 回调分发函数声明（通过 include.h → Service.h → Apply_lin.h 也可获取） */
extern void lin_pid_resp_callback_handler(uint8_t iii, uint8_t event_id, uint8_t id);
	...
	else
	{
		...
		if(pTpChannel != NULL_PTR)
		{
			...
			if(LinTp_SlaveRunCfg[u16ChIndex].LinTpTxStatus == LINTP_TX_CONFIRMATION_STATUS)
			{
				...
				PduR_LinTpTxConfirmation(pduId, E_OK);
			}
			else
			{
				...
				if(pLinIfFrame->LinIfFrameType == LINIF_SRF)
				{
					...
					if(pTpChannel != NULL_PTR)
					{
						...
						if(LinTp_SlaveRunCfg[u16ChIndex].LinTpTxStatus == LINTP_TX_CONFIRMATION_STATUS)
						{
							 ...
							 */
							LinTp_ResetChannel(&LinTp_SlaveRunCfg[u16ChIndex]);
							PduR_LinTpTxConfirmation(pduId, E_NOT_OK);
						}
					}
				}
#endif /* LINIF_TP_SUPPORTED == STD_ON */
...
)
{
     ...
     */
    bufResult = PduR_LinTpStartOfReception(
    		pduId,
			info,
    ...
    if ((BUFREQ_OK == bufResult) && (LinTp_SlaveRunCfg[Channel].UpperBufRemaining >= copyLen))
    {
    	 ...
    	 */
    	bufResult = PduR_LinTpCopyRxData(
    			pduId,
				info,
				&(LinTp_SlaveRunCfg[Channel].UpperBufRemaining));
    	...
    	if(BUFREQ_OK == bufResult)
    	{
    		...
    		PduR_LinTpRxIndication(pduId, E_OK);
    	}
    	else if(BUFREQ_E_NOT_OK == bufResult)
    	{
        	 ...
        	 */
			LinTp_ResetChannel(&LinTp_SlaveRunCfg[Channel]);
    		PduR_LinTpRxIndication(pduId, E_NOT_OK);
    	}
    	else
    	{
    		/* do nothing .... */
...
)
{
	...
	if(LinTp_SlaveRunCfg[Channel].u8CopyCnt <= LinTp_SlaveRunCfg[Channel].LinTpTxNSdu->LinTpMaxBufReq)
	{
		...
		bufResult = PduR_LinTpCopyTxData(
	    		pduId,
				info,
				NULL_PTR,
				&(LinTp_SlaveRunCfg[Channel].UpperBufRemaining));
...
#endif /* LINIF_DEV_ERROR_DETECT == STD_ON */
			{
				if((Lin_SduPtr[0]==0x7E)&&(Lin_SduPtr[1]==0x02)&&(Lin_SduPtr[2]==0x3E)&&(Lin_SduPtr[3]==0x80))
				{
...
)
{
	...
	if((Std_ReturnType)E_OK == retStatus)
	{
...
#endif /* LINIF_DEV_ERROR_DETECT == STD_ON */
		{
			...
			/* report error to Lin Geny */
			VAR(uint8,AUTOMATIC) lin_geny_event;
			switch((uint8)ErrorStatus)
			{
				case LIN_ERR_HEADER:       lin_geny_event = 3U;  break;
				case LIN_ERR_RESP_STOPBIT: lin_geny_event = 4U;  break;
				case LIN_ERR_RESP_CHKSUM:  lin_geny_event = 5U;  break;
				case LIN_ERR_NO_RESP:      lin_geny_event = 13U; break;
				case LIN_ERR_RESP_DATABIT:  lin_geny_event = 6U;  break;
				case LIN_ERR_INC_RESP:     lin_geny_event = 11U; break;
				default:                   lin_geny_event = 0xFFU; break;
			}
			if(lin_geny_event != 0xFFU)
			{
				lin_pid_resp_callback_handler((uint8)u16ChIdex, lin_geny_event, 0xFFU);
			}
		}
#if(LINIF_DEV_ERROR_DETECT == STD_ON)
	}
                                      ...
                                      uint8_t txSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            linCurrentState->txSize = (uint8_t)(txSize);
                                    ...
                                    uint8_t rxSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Add a place for checksum byte */
            linCurrentState->rxSize = (uint8_t)(rxSize);
