 ...
 *
 ****************************************************************************/
//extern ResultStatus_t Eep_EraseSector(uint32_t addr, const FLASH_CmdConfig_t *config);
uint32_t EE_SectorErase(uint32_t addr, uint32_t size, void (*CallBack)(void))
{
    uint32_t returnCode = EE_OK;
    uint32_t counter;
    ResultStatus_t tmp;
   Flash_Drv_CmdActionType cmdConfig =
   {
       FALSE,
       NULL_PTR,
       NULL_PTR,
    ...
    {           
        tmp = Flash_Drv_EraseSector(addr + counter*EE_FL_SECTOR_SIZE, &cmdConfig);
...
//    };
    ...
    {        
        alignedLen = (len / EE_FL_PHRASE_SIZE) * EE_FL_PHRASE_SIZE;
			//  alignedLen = (len / EE_FL_PHRASE_SIZE) * EE_FL_PHRASE_SIZE + 1;
        ...
        while(alignedLen != 0U)
        {
            if(srcInFlash == 1U)
            {
                ...
                if(FLASH_DRV_ERR == FLASH_ProgramDflashPhrase(writeAddr, buf, &CmdAct))
                {
                    returnCode = EE_ERROR_FLASH_ERR;
                }
            }
            ...
            {
                if(FLASH_DRV_ERR == FLASH_ProgramDflashPhrase(writeAddr, (uint8 *)readAddr, &CmdAct))
                {
                    returnCode = EE_ERROR_FLASH_ERR;
                }
            }            
...
//        }
        ...
        if(returnCode == EE_OK)
        {
            if(remaind != 0U)
            {
               ...
               if(ERR == FLASH_ProgramDflashPhrase(writeAddr,  buf, &CmdAct))
               {
                   returnCode = EE_ERROR_FLASH_ERR;
               }
            }
