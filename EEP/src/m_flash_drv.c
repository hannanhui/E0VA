...
static const uint32 flashInterruptMaskTable[] =
{
...
static ResultStatus_t FLASH_Compare(const uint32 srcAddr, const uint8_t * const compareBuff,
                                                                            const uint32 length);
#endif

#if(1 == FLASH_ERASE_VERIFICATION_ENABLED)
static ResultStatus_t FLASH_SectorBlankCheck(const uint32 srcAddr);
#endif
...
static ResultStatus_t FLASH_ExecuteCommand(FLASH_Cmd_t cmd, flash_cb_t callBack)
{
    ...
    else
    {
        ...
        pFlashReg->FLASH_FCMD.CMDCODE = (uint32)cmd;
 ...
 /*       ASMV_KEYWORD(
        ...
        */
        ...
        scmRegPtr->SCM_MISCCTL1.CACHE_DIS = (uint32)oldCacheStat;
...
static ResultStatus_t Eep_ExecuteCommand(FLASH_Cmd_t cmd, flash_cb_t callBack)
{
    ResultStatus_t stat = SUCC;
    uint32 cmdAddr;
    uint32 regOldPriMask;
...
static ResultStatus_t FLASH_SectorBlankCheck(const uint32 srcAddr)
{
    ...
    else
    {
        ...
        { 
            if (*(volatile uint32 *)(srcAddr + lenCount) != FLASH_32BIT_MAX_VALUE)
            {
                stat = ERR;
                break;
            }
 ...
 *
 */
void FLASH_IgnoreBusErrorConfig(uint32 startAddr, uint32 endAddr)
{
 ...
 *
 */   
ResultStatus_t FLASH_VerifyBlock(uint32 addr,
 ...
 *
 */   
ResultStatus_t FLASH_VerifySector(uint32 addr,
                                  const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_VerifyPage(uint32 addr,
                                const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_VerifyPhrase(uint32 addr,
                                  const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_PagesMircSignature(uint32 startAddr, uint32 endAddr,
                                        uint32 seed[4], uint32 signature[4],
                                        const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_VerifyIfrSector(uint32 addr,
                                     const FLASH_CmdConfig_t *config)   
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_VerifyIfrPage(uint32 addr,
                                   const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_VerifyIfrPhrase(uint32 addr,
                                     const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */   
ResultStatus_t FLASH_IfrPagesMircSignature(uint32 startAddr, uint32 endAddr,
                                        uint32 seed[4], uint32 signature[4],
                                        const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
 ...
 *
 */
ResultStatus_t FLASH_ProgramDflashPhrase(uint32 flashAddr, const uint8_t *dataP, 
                                                   const FLASH_CmdConfig_t *config)
{
    ...
    else
    {
        ...
        if(FLASH_CMD_ACT_WAIT == config->act)
        {
            stat = Eep_ExecuteCommand(FLASH_CMD_PGMPHR, config->callBack);
#if(1 == FLASH_PROGRAM_VERIFICATION_ENABLED)
            if(stat != ERR)
            {
 ...
 *
 */
ResultStatus_t FLASH_ProgramPhrase(uint32 flashAddr, const uint8_t *dataP, 
                                   const FLASH_CmdConfig_t *config)
{
    volatile uint8_t *fData = (volatile uint8_t *)&(flsRegWPtr->FLASH_FDATA0);
    ResultStatus_t stat;
 ...
 *
 */   
ResultStatus_t FLASH_Program(uint32 flashAddr, uint32 len, const uint8_t *dataP,
                             flash_cb_t callBack)
{
    uint32 count;
    uint32 bufAddr;
...
ResultStatus_t Eep_Program(uint32 flashAddr, uint32 len, const uint8_t *dataP,
                             flash_cb_t callBack)
{
    uint32 count;
    uint32 bufAddr;
    ...
    else
    {
        ...
        {
            /*PRQA S 0306 ++*/
            stat = Eep_ProgramPhrase((flashAddr + count), (uint8_t *)(bufAddr + count),
                                       &config);
            /*PRQA S 0306 --*/
            if(SUCC != stat)
            {
 ...
 *
 */
ResultStatus_t FLASH_EraseDflashSector(uint32 addr, const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
    ...
    else
    {
        ...
        if(FLASH_CMD_ACT_WAIT == config->act)
        {
            stat = Eep_ExecuteCommand(FLASH_CMD_ERSSCR, config->callBack);
#if(1 == FLASH_ERASE_VERIFICATION_ENABLED)
            if(stat != ERR)
            {
 ...
 *
 */
ResultStatus_t FLASH_EraseSector(uint32 addr, const FLASH_CmdConfig_t *config)
{
    ResultStatus_t stat;
    uint32 ccifFlag;
 ...
 *
 */ 
void FLASH_InjectEcc(uint32 addr, uint32 eccBits, uint32 dataBits[4])
{
    uint32 lockFlag;
 ...
 *
 */ 
FlagStatus_t FLASH_GetStatus(uint32 statMask)
{
    FlagStatus_t status;
    
    status = (((flsRegWPtr->FLASH_FSTAT) & (uint32)statMask) == 0U)? RESET: SET;
...
{
    ResultStatus_t stat = SUCC;
    uint32 failFlag = flsRegPtr->FLASH_FSTAT.FAIL;
...
uint8_t Flash_Write(uint32 flashAddr, uint32 len, const uint8_t *dataP)
{
	ResultStatus_t stat = ERR;
	uint32 Len16=0;
	uint8_t data[16]={0};
	...
	if(len%16!=0)
	{
		...
		{
			Len16=(len/16)*16;
			stat=FLASH_Program(flashAddr,Len16,dataP,NULL);
			if(stat==ERR)
			{
				return stat;
			}
			
			memcpy(data,(dataP+Len16),len-Len16);
			    	for(volatile int i=0;i<1000;i++)
			{
				
			}
			stat=FLASH_Program(flashAddr+Len16,16,data,NULL);
		}
		else 
		{
			...
			stat=FLASH_Program(flashAddr,16,data,NULL);
		}
	...
	else
	{
		...
		stat=FLASH_Program(flashAddr,len,dataP,NULL);
	}
	
	return stat;
}

uint8_t Flash_EraseNSectors(uint32 addr,uint32 sectors)
{
	ResultStatus_t stat = ERR;
	if(addr%1024!=0)
	{
	...
	for(int i=0;i<sectors;i++)
	{
	   ...
	   stat=FLASH_EraseSector(addr,NULL);
		 addr+=8192;
	}
...
uint8_t DFlash_Write(uint32 flashAddr, uint32 len, const uint8_t *dataP)
{
	ResultStatus_t stat = ERR;
	uint32 Len16=0;
	uint8_t data[16]={0};
	
	if(flashAddr<0x01000000||flashAddr>0x01020000)
	{
		return 1;
	}
	...
	if(len%16!=0)
	{
		...
		{
			Len16=(len/16)*16;
			stat=Eep_Program(flashAddr,Len16,dataP,NULL);
			if(stat==ERR)
			{
				return stat;
			}
			...
			stat=Eep_Program(flashAddr+Len16,16,data,NULL);
		}
		else 
		{
			memcpy(data,dataP,len);
			...
			stat=Eep_Program(flashAddr,16,data,NULL);
		}
...
uint8_t DFlash_EraseNSectors(uint32 addr,uint32 sectors)
{
	ResultStatus_t stat = ERR;
	...
	for(int i=0;i<sectors;i++)
	{
	   ...
	   stat=FLASH_EraseDflashSector(addr,NULL);
