...
#define MCALLIB_OSCOUNTER_C_SW_MAJOR_VERSION            2U
#define MCALLIB_OSCOUNTER_C_SW_MINOR_VERSION            0U
#define MCALLIB_OSCOUNTER_C_SW_PATCH_VERSION            0U
...
#define Z20_SYSTICK_CTRL_ENABLE_MASK ((uint32)(1U << Z20_SYSTICK_CTRL_ENABLE_POS))

#define Z20_SYSTICK_CTRL_CLKSOURCE_POS  (2U)
#define Z20_SYSTICK_CTRL_CLKSOURCE_MASK ((uint32)(1U << Z20_SYSTICK_CTRL_CLKSOURCE_POS))
...
/**
 * @brief Defines conversion factor between seconds and microseconds
 */
#define MCALLIB_CONVSERSION_FACTER_S_BETWEEN_US (1000000U)
 ...
 */
    ...
    #if (STD_ON == MCALLIB_DEV_ERROR_DETECT)
static void McalLib_DetReportError(const uint8 ServiceId, const uint8 ErrorId);
    #endif /* STD_ON == MCALLIB_DEV_ERROR_DETECT */
...
static void   McalLib_Systick_GetElapsedValue(uint32 *const CounterValue, 
                                              uint32 *const ElapsedValue);
    #endif
 ...
 */
    ...
    #if (STD_ON == MCALLIB_DEV_ERROR_DETECT)
/**
 * @brief     McalLib wrapper function for Det_ReportError()
 *
 * @param[in]  ServiceId: Service ID of the error
 * @param[in]  ErrorId: Error ID of the error
 *
 * @return    None
 *
 */
...
static void McalLib_Systick_Init(void)
{
    ...
    const uint32 ModeSwitched = Sys_EnterPrivilegedMode();
        #endif
    /* set reload value */
    ...
    Z20_SysTickRegPtr->CTRL =
        (uint32)((((uint32)((uint32)(MCALLIB_SYSTEM_TICK_TIMER_CLKSOURCE)
                            << Z20_SYSTICK_CTRL_CLKSOURCE_POS)) &
                  Z20_SYSTICK_CTRL_CLKSOURCE_MASK) |
                 (((uint32)((1UL) << Z20_SYSTICK_CTRL_ENABLE_POS)) & Z20_SYSTICK_CTRL_ENABLE_MASK));
...
static uint32 McalLib_Systick_GetCounterValue(void)
{
    ...
    const uint32 ModeSwitched = Sys_EnterPrivilegedMode();
        #endif
    const uint32 Value = Z20_SysTickRegPtr->VAL;
...
/**
 * @brief     Get the elapsed value of the SysTick counter
 *
 ...
 *
 */
static void McalLib_Systick_GetElapsedValue(uint32 *const CounterValue, uint32 *const ElapsedValue)
{
        #if (MCALLIB_ENABLE_USER_MODE_SUPPORT == STD_ON)
    const uint32 ModeSwitched = Sys_EnterPrivilegedMode();
        #endif
    const uint32 CurrentValue = Z20_SysTickRegPtr->VAL;

    if (CurrentValue > *CounterValue)
    {
        /* counter overflow */
        *ElapsedValue = ((Z20_SysTickRegPtr->LOAD + *CounterValue) - CurrentValue) + 1U;
    }
    else
    {
    ...
    *CounterValue = CurrentValue;
    ...
    #endif /*MCALLIB_OS_USED == MCALLIB_OS_BAREMETAL OR MCALLIB_OS_USED == MCALLIB_OS_FREERTOS*/
...
{
    const uint32 CoreId = McalLib_GetCoreID();
 ...
 *
 */
Std_ReturnType McalLib_OsCounter_GetCounterValue(uint32 *const Value)
{
 ...
 *
 */
Std_ReturnType McalLib_OsCounter_GetElapsedValue(uint32 *const CounterValue, 
                                                 uint32 *const ElapsedValue)
{
    Std_ReturnType   RetValue;
    const uint32     CoreId = McalLib_GetCoreID();
 ...
 *
 */
uint32 McalLib_OsCounter_MicroSecToTicks(const uint32 MicroSecond)
{
    uint32 Ticks = 0U;
    uint64 Temp;
    const uint32 CoreId = McalLib_GetCoreID();
    ...
    else
    {
        ...
        else
        {
            ...
            Ticks = (uint32)(Temp / MCALLIB_CONVSERSION_FACTER_S_BETWEEN_US);
    #if (STD_ON == MCALLIB_DEV_ERROR_DETECT)
        }
    }
    #endif /* STD_ON == MCALLIB_DEV_ERROR_DETECT */
 ...
 *
 */
void McalLib_OsCounter_SetCounterFreq(const uint32 Freq)
{
    const uint32 CoreId = McalLib_GetCoreID();
    ...
    (void)CoreId;
    (void)Freq;
    #endif /* MCALLIB_OS_USED == MCALLIB_OS_FREERTOS OR MCALLIB_OS_USED == MCALLIB_OS_BAREMETAL */

    #if (STD_ON == MCALLIB_DEV_ERROR_DETECT)
    }
    #endif /* STDSTD_ON == MCALLIB_DEV_ERROR_DETECT */
}
