...
#define FEE_C_SW_PATCH_VERSION 0u
/*******************************************************************************
**                       Version  Check                                       **
*******************************************************************************/
...
#define FEE_DESERIALIZE(pDeserialPtr, ParamVal, ParamType) \
    do                                                     \
    {                                                      \
        (ParamVal) = *((const ParamType*)(pDeserialPtr));  \
...
#define FEE_DET_REPORT_ERR(api, error) Det_ReportError(FEE_MODULE_ID, FEE_INSTANCE_ID, (api), (error));
#endif /* FEE_DEV_ERROR_DETECT == STD_ON */

#define FEE_DET_REPORT_RUNTIME_ERR(api, error) Det_ReportRuntimeError(FEE_MODULE_ID, FEE_INSTANCE_ID, (api), (error));
/*******************************************************************************
    ...
    VAR(uint16, AUTOMATIC) Length)
{
    ...
    else
    {
        ...
        if ((MEMIF_BUSY != Fee_ModuleStatus) && (MEMIF_BUSY_INTERNAL != Fee_ModuleStatus))
        {
            SchM_Enter_Fee(FEE_INSTANCE_ID, FEE_AREA_MSGCONTEXT);
            ...
            SchM_Exit_Fee(FEE_INSTANCE_ID, FEE_AREA_MSGCONTEXT);
            ret = E_OK;
        }
        else
        {
    ...
    P2VAR(uint8, AUTOMATIC, FEE_APPL_DATA) BlockHdrPtr)
{
    ...
    uint8* endPtr;
    endPtr = startPtr + FEE_BLOCK_HEADER_SIZE;
    ...
    P2CONST(uint8, AUTOMATIC, FEE_APPL_CONST) BlockHdrPtr)
{
    ...
    {
#if (STD_ON == FEE_PAGE_DIRECT_WRITE_SUPPORT)
        endPtr = (blockHdrPtr + FEE_BLOCK_HEADER_SIZE);
#else
...
#if (STD_ON == FEE_PAGE_DIRECT_WRITE_SUPPORT)
        else if (((Std_ReturnType)E_OK) != Fee_BlankCheck(blockHdrPtr, (endPtr - 2u)))
        {
            /* FEE_BLOCK_HEADER_INVALID */
        }
