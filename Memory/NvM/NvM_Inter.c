...
#include "NvM_MemMap.h"
VAR(uint8, NVM_VAR_NOINIT) NVM_RamMirror[NVM_MAX_LENGTH_CONFIGED_RAM_MIRROR]; /*PRQA S 1514*/
...
static void NvM_MemIfReqSubDeal(NvM_MemIfReqType NvM_MemIfRequest)
{
    ...
    case NVM_MEMIF_REQ_READ:
        ...
        if ((Std_ReturnType)E_OK == MemIf_Read(NvM_CurRunning.DeviceId, l_BlockNum, 0, NvM_NvDataBuffer, l_Length))
        {
    ...
    NvM_MemCpy(P2CONST(uint8, AUTOMATIC, NVM_VAR) SrcAddrPtr, P2VAR(uint8, AUTOMATIC, NVM_VAR) DstAddrPtr)
{
    ...
    {
        NOT = NvM_GetWordBitState(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup, NVM_ADMIN_RAMMIRROR_OR_NOT);
        if ((uint8)STD_ON == NOT)
        {
...
#endif
            {
                if ((Std_ReturnType)E_OK == (*NvM_CurRunning.NvM_WriteRamBlockToNvm)((void*)SrcAddrPtr)) /*PRQA S 0311*/
                {
...
{
    uint8 SYNCMECHANISM;
    NvM_CurRunning.ManagementType = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmBlockManagementType;
#if (NVM_API_CONFIG_CLASS_1 != NVM_API_CONFIG_CLASS)
    ...
    NvM_CurRunning.WriteTimesCounter =
        NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvMMaxNumOfWriteRetries + 1U; /*req 659*/
    NvM_CurRunning.ReadRetryCounter =
        NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvMMaxNumOfReadRetries; /*req 533*/
    NvM_CurRunning.VerificationSize = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvMWriteVerificationDataSize;
    ...
    NvM_CurRunning.BaseNumber = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmNvBlockBaseNumber;
    ...
    if (NVM_BLOCK_DATASET != NvM_CurRunning.ManagementType)
    {
        NvM_CurRunning.RomAddr = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRomBlockDataAddress;
    }
    ...
    {
        NvM_CurRunning.NvM_WriteRamBlockToNvm = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvM_WriteRamBlockToNvm;
        NvM_CurRunning.NvM_ReadRamBlockFromNvm =
            NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvM_ReadRamBlockFromNvm;
    }
    /***********************************/

#if (NVM_API_CONFIG_CLASS_1 != NVM_API_CONFIG_CLASS)
    NvM_CurRunning.InitCallback = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmInitBlockCallback;
#endif
    NvM_CurRunning.SingleCallback = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmSingleBlockCallback;
}
...
NvM_JobEnqueue(NvM_BlockIdType BlockId, NvM_ServiceIdType ServiceId, P2VAR(uint8, AUTOMATIC, NVM_APPL_DATA) DestSrcPtr)
{
    ...
    SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA) if (NVM_SIZE_STANDARD_JOB_QUEUE > NvM_StandQueueManage.Count)
    {
    ...
    SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA) NvM_CurRunning.BlockId = BlockId;
    NvM_MultiJob.Enqueue = STD_ON;
    ...
    l_ReturnValue = E_OK;
#endif
    SchM_Exit_NvM(0, INTERRUPT_PROTECTION_AREA) return l_ReturnValue;
}
...
FUNC(Std_ReturnType, NVM_CODE) NvM_JobDequeue(void)
{
    ...
    else if (0U < NvM_StandQueueManage.Count)
    {
        ...
        NvM_StandQueueManage.Count--;
        SchM_Exit_NvM(0, INTERRUPT_PROTECTION_AREA) l_ReturnValue = E_OK;
    }
    else
    {
    ...
    {
        SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA) NvM_Module.CurrentJobType = NVM_JOB_TYPE_MULTI;
        if (NVM_READ_ALL_SERV_ID == NvM_CurRunning.ServiceId)
        {
            NvM_CurRunning.BlockId = NVM_READALL_FIRST_BLOCKID;
        }
    ...
    NvM_GetQueueSpace(NvM_JobTypeType JobType, P2VAR(uint8, AUTOMATIC, NVM_VAR) JobQueueIndexPtr)
{
    ...
    if (NVM_JOB_TYPE_SINGLE_IMMED == JobType)
    {
        ...
        {
            SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA)* JobQueueIndexPtr = NvM_ImmedQueueManage.TailIndex;
    ...
    else if (NVM_JOB_TYPE_SINGLE_NORMAL == JobType)
    {
        ...
        else
        {
            ...
            l_JobQueueIndex = (uint8)(l_JobQueueGroup << 4u) + l_JobQueueRdyBit; /*PRQA S 2986*/
            ...
            {
                SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA)* JobQueueIndexPtr = l_JobQueueIndex;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA)* JobQueueIndexPtr = l_SamePriHeadIndex;
...
FUNC(Std_ReturnType, NVM_CODE) NvM_JobDequeue(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA) if ((Std_ReturnType)E_OK == l_ReturnValue)
            {
...
#endif
    {
        REPAIR = NvM_GetWordBitState(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup, NVM_ADMIN_NV_REPAIR);
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_JobOverSetFlag(NvM_RequestResultType SingleReqResult, NvM_CrcChangeType CrcChange)
{
    ...
    if (NULL_PTR != NvM_CurRunning.SingleCallback)
    {
        ...
        case NVM_ERASE_NV_BLOCK_SERV_ID:
            BlockRequest = NVM_INIT_READ_ALL_BLOCK;
            break;
...
static FUNC(void, NVM_CODE) NvM_MemIfReadFailed(void)
{
    ...
    {
        NvM_UpdateValidandChangeStatus(STD_OFF, STD_OFF);
#if (STD_ON == NVM_DEM_E_REQ_FAILED)                                      /*PRQA S 3332*/
        Dem_ReportErrorStatus(NVM_E_REQ_FAILED, DEM_EVENT_STATUS_FAILED); /* req 678  654  781*/
#endif
#if (STD_ON == NVM_DEM_E_HARDWARE)                                      /*PRQA S 3332*/
...
static FUNC(void, NVM_CODE) NvM_MemIfInconsistent(void)
{
    ...
    {
        NvM_UpdateValidandChangeStatus(STD_OFF, STD_OFF);
#if (STD_ON == NVM_DEM_E_INTEGRITY_FAILED) /*PRQA S 3332*/
...
static FUNC(void, NVM_CODE) NvM_MemIfInvalid(void)
{
    ...
    else if ((NVM_BLOCK_NATIVE == NvM_CurRunning.ManagementType) || (NVM_JOB_STEP_READ_2ND_NV == NvM_Module.JobStep))
#else
    if ((NVM_BLOCK_NATIVE == NvM_CurRunning.ManagementType) || (NVM_JOB_STEP_READ_2ND_NV == NvM_Module.JobStep))
#endif
    {
        ...
        {
            NvM_UpdateValidandChangeStatus(STD_OFF, STD_OFF);
            NvM_JobOverSetFlag(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult, NVM_CRC_DELETE);
        }
    }
    else
    {
...
FUNC(void, NVM_CODE) NvM_InterReadRom(void)
{
    ...
    if (NVM_JOB_STEP_CALC_CRC == NvM_Module.JobStep)
    {
        ...
        if ((NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult == NVM_REQ_INTEGRITY_FAILED)
            || (NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult == NVM_REQ_NV_INVALIDATED)
...
FUNC(void, NVM_CODE) NvM_InterCalcCrc(void)
{
    ...
    {
        NvM_CurRunning.CrcFlag = 1U; /*PRQA S 2982*/
    ...
    if (NVM_CRC_NUM_OF_BYTES < NvM_CurRunning.CrcLength) /*PRQA S 2996,2992*/
    {
        l_CrcLength = NVM_CRC_NUM_OF_BYTES; /*PRQA S 2880*/
    }
    else
    ...
    NvM_CurRunning.CrcAddr += l_CrcLength; /*PRQA S 0488*/
}
...
static FUNC(void, NVM_CODE) NvM_MemCpyStep(void)
{
    uint8 NOT = NvM_GetWordBitState(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup, NVM_ADMIN_RAMMIRROR_OR_NOT);
    uint8 LOSS;
...
#if (STD_ON == NVM_DEM_E_LOSS_OF_REDUNDANCY) /*PRQA S 3332*/
...
#endif
#if (STD_ON == NVM_DEM_E_VERIFY_FAILED)                                              /*PRQA S 3332*/
    ...
    NvM_ReadErrorHandleRedundant(NvM_CrcStaticIdStatusType CRCResult, NvM_CrcStaticIdStatusType StaticIDResult)
{
    ...
    else
    {
            ...
            || (NULL_PTR != NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmInitBlockCallback))
        {
            ...
            {
#if (STD_ON == NVM_DEM_E_WRONG_BLOCK_ID) /*PRQA S 3332*/
...
static FUNC(void, NVM_CODE) NvM_MultiJobResultFeedBack(NvM_ServiceIdType ServiceId, NvM_RequestResultType Result)
{
    ...
    NvM_MultiBlockRequestType NotiServiceId;
    switch (ServiceId)
    {
    case NVM_READ_ALL_SERV_ID:
        NotiServiceId = NVM_READ_ALL;
        break;
