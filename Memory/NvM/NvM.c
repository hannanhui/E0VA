...
#define NVM_C_AR_MINOR_VERSION 5U
...
/* req NVM089 */
#if (                                                                                                           \
    (NVM_C_AR_MAJOR_VERSION != NVM_H_AR_MAJOR_VERSION) || (NVM_C_AR_MINOR_VERSION != NVM_H_AR_MINOR_VERSION)    \
    || (NVM_C_AR_PATCH_VERSION != NVM_H_AR_PATCH_VERSION) || (NVM_C_SW_MAJOR_VERSION != NVM_H_SW_MAJOR_VERSION) \
    || (NVM_C_SW_MINOR_VERSION != NVM_H_SW_MINOR_VERSION)                                                       \
    || (NVM_C_SW_PATCH_VERSION != NVM_H_SW_PATCH_VERSION) /*cfg check*/                                         \
    || (2 != NVM_CFG_H_SW_MAJOR_VERSION) || (0 != NVM_CFG_H_SW_MINOR_VERSION) || (0 != NVM_CFG_H_SW_PATCH_VERSION))
...
FUNC(void, NVM_CODE) NvM_Init(const NvM_ConfigType* ConfigPtr) /*PRQA S 3206*/
{
    ...
    for (l_BlockIdLoop = 0; l_BlockIdLoop <= l_LoopCount; l_BlockIdLoop++) /*PRQA S 2877*/
    {
        if (l_BlockIdLoop < l_LoopCount) /*PRQA S 2996,2992*/
        {
            NvM_QueueSpaceTalbe[l_BlockIdLoop] = 0xFFFFu; /*PRQA S 2880*/
...
#endif
#if (STD_ON == NVM_DEM_E_WRITE_PROTECTED) /*PRQA S 3332*/
...
FUNC(Std_ReturnType, NVM_CODE) NvM_EraseNvBlock(NvM_BlockIdType BlockId)
{
...
#endif
    {
        ...
        if ((uint8)STD_ON == ENQUEUE)
        {
            ...
            || ((0U != NvM_BlockDescriptor[BlockId - 1U].NvmBlockJobPriority) && (STD_ON == NVM_JOB_PRIORITIZATION)))
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            /* req NVM357 */
...
#define NVM_START_SEC_CODE
#include "NvM_MemMap.h"
FUNC(Std_ReturnType, NVM_CODE) NvM_SetRamBlockStatus(NvM_BlockIdType BlockId, boolean BlockChanged) /* req NVM453 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 WRPRO;
    uint8 ENQUEUE;
...
FUNC(Std_ReturnType, NVM_CODE)
NvM_GetDataIndex(NvM_BlockIdType BlockId, P2VAR(uint8, AUTOMATIC, NVM_VAR) DataIndexPtr) /* req NVM449 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
 ...
 */
/* req NVM027 NVM191 NVM409 */
FUNC(Std_ReturnType, NVM_CODE) NvM_ReadBlock(NvM_BlockIdType BlockId, void* NvM_DstPtr) /* req NVM454 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
 ...
 */
/* req NVM027 NVM191 NVM409 */
FUNC(Std_ReturnType, NVM_CODE) NvM_WriteBlock(NvM_BlockIdType BlockId, void* NvM_SrcPtr)
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
 ...
 */
/* req NVM027 NVM191 NVM409 */
FUNC(Std_ReturnType, NVM_CODE) NvM_RestoreBlockDefaults(NvM_BlockIdType BlockId, void* NvM_DestPtr)
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_NormalQueueSort(NvM_BlockIdType BlockId)
{
    ...
    else
    {
        ...
        if (((0 != NvM_StandQueueManage.TailIndex) && (l_BlockIdIndex == (NvM_StandQueueManage.TailIndex - 1)))
            || ((0 == NvM_StandQueueManage.TailIndex) && (l_BlockIdIndex == (NVM_SIZE_STANDARD_JOB_QUEUE - 1))))
        {
...
{
    uint8 l_JobPri;
    uint8 l_Preindex = 0; /*PRQA S 2981*/
    uint8 l_Curindex = 0;
    uint8 l_BlockIdIndex = 0; /*PRQA S 2981*/
    uint8 l_QueueCounter = 0;
    ...
    else
    {
        ...
        if (ret == E_OK)
        {
            ...
            NvM_QueueSpaceTalbe[((l_BlockIdIndex & 0xF0U) >> 4u)] |=
                (uint16)((uint16)0x01U << (l_BlockIdIndex & 0x0FU)); /*PRQA S 2985*/
        }
    }
    return ret;
}
...
FUNC(void, NVM_CODE) NvM_MainFunction(void)
{
    ...
    if ((uint8)STD_ON == NvM_Module.Init)
    {
        ...
        if (0U != NvM_AtomJob.Count)
        {
            ...
            if ((NVM_JOB_STEP_IDLE == NvM_Module.JobStep) && (NVM_JOB_TYPE_MULTI == NvM_Module.CurrentJobType)
                && ((NVM_REQ_OK != NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult)&&(NVM_REQ_BLOCK_SKIPPED != NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult)))
            {
