...
#define FLI_OFFSET   (4) /* tlsf structure just will manage blocks bigger */
/* than 128 bytes */
...
static __inline__ bhdr_t* process_area(void* area, size_t size)
{
    ...
    ib = (bhdr_t*)area;
    ib->size = (sizeof(area_info_t) < MIN_BLOCK_SIZE) ? MIN_BLOCK_SIZE
...
size_t add_new_area(void* area, size_t area_size, void* mem_pool)
{
    ...
    while (ptr)
    {
        ...
        if ((unsigned long)ib1 == (unsigned long)lb0 + BHDR_OVERHEAD)
        {
            ...
            b0->size = ROUNDDOWN_SIZE((b0->size & BLOCK_SIZE) + (ib1->size & BLOCK_SIZE) + 2 * BHDR_OVERHEAD)
        ...
        if ((unsigned long)lb1->ptr.buffer == (unsigned long)ib0)
        {
            ...
            lb1->size = ROUNDDOWN_SIZE((b0->size & BLOCK_SIZE) + (ib0->size & BLOCK_SIZE) + 2 * BHDR_OVERHEAD)
