...
#define CRYIF_C_SW_MINOR_VERSION 0U
...
*******************************************************************************/
#if (STD_ON == CRYIF_DEV_ERROR_DETECT)
#define CRYIF_DET_REPORT(ApiId, ErrorId) ((void)Det_ReportError(CRYIF_MODULE_ID, CRYIF_INSTANCE_ID, (ApiId), (ErrorId)))
#endif
...
CryIf_ProcessJob(VAR(uint32, CRYIF_APPL_DATA) channelId, P2VAR(Crypto_JobType, AUTOMATIC, CRYIF_APPL_DATA) job)
{
...
#endif
    {
        if (CRYPTO_HASH == job->jobPrimitiveInfo->primitiveInfo->service)
        {
            ...
            || (CRYPTO_ENCRYPT == job->jobPrimitiveInfo->primitiveInfo->service)
            ...
            || (CRYPTO_SIGNATUREVERIFY == job->jobPrimitiveInfo->primitiveInfo->service)
        ...
        {
            if (job->jobPrimitiveInfo->cryIfKeyId >= CRYIF_KEY_MAX_CONFIGURED)
            {
#if (STD_ON == CRYIF_DEV_ERROR_DETECT)
                CRYIF_DET_REPORT(CRYIF_SID_PROCESSJOB, CRYIF_E_PARAM_HANDLE);
#endif
            ...
            {
                job->cryptoKeyId = CryIf_GetDriverKeyId(job->jobPrimitiveInfo->cryIfKeyId);
                ret = E_OK;
            }
            /* @req [SWS_CryIf_00135] */
            if (CRYPTO_KEYDERIVE == job->jobPrimitiveInfo->primitiveInfo->service)
            {
