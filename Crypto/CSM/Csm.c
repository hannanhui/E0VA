...
#define CSM_C_SW_MINOR_VERSION 0U
#define CSM_C_SW_PATCH_VERSION 0U
...
#define CSM_START_SEC_VAR_CLEARED_BOOLEAN
#include "Csm_MemMap.h"
VAR(boolean, CSM_VAR_CLEARED_BOOLEAN) Csm_InitStatus;
...
Csm_Init(P2CONST(Csm_ConfigType, AUTOMATIC, CSM_APPL_DATA) configPtr)
{
...
#endif /*(CSM_DEV_ERROR_DETECT == STD_ON)*/
    {
        ...
        /*Init job-queue*/
        for (queIdx = 0u; queIdx < CSM_QUEUE_NUM; queIdx++)
        {
    ...
    VAR(uint32, AUTOMATIC) jobId)
{
    ...
    {
        /*[SWS_Csm_91014]*/
        if ((jobPrimInOut->inputLength != 0u) || (*(jobPrimInOut->outputLengthPtr) != 0u))
        {
            ret = (boolean)E_NOT_OK;
            goto Csm_ProcessJobToCryIfExit;
        }
    }
    ...
    else /*Job in CRYPTO_JOBSTATE_IDLE state*/
    {
        ...
        if (CRYPTO_PROCESSING_SYNC == processMode)
        {
            ...
            if (((jobCfgPtr->jobInfo->jobPriority > queChRunPtr->maxPriority)) || (0u == queChRunPtr->jobCnt))
            {
                ...
                ret = CryIf_ProcessJob(channelId, &job);
                if (((boolean)E_OK == ret)
                    && (0u == (jobPrimInOut->mode & CRYPTO_OPERATIONMODE_FINISH)) /*PRQA S 4522,1823*/
                )
                {
