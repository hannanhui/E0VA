...
STATIC FUNC(uint8, CRC_CODE) Crc_GetReflection_uint8(VAR(uint8,AUTOMATIC) data)
{
	uint8 i,reflection = 0u;
	for(i = 0u;i < 0x08u; i++)
	{
		if((data & (uint8)0x01) != 0u )
		{
			reflection |= (uint8)(1u << (7u - i));
		}
		data >>=1u;
	}
	return reflection;
...
STATIC FUNC(uint32, CRC_CODE) Crc_GetReflection_uint32(VAR(uint32,AUTOMATIC) data)
{
	uint32 i,reflection = 0u;
	for(i = 0u;i < 0x20u; i++)
	{
		if((data & (uint32)0x01) != 0u )
		{
			reflection |= (uint32)((uint32)1u << (uint32)(0x1fu - i));	/* PRQA S 2791 EOF # violate MISRA_2004_Rule_21.1:Logical need here */ /* PRQA S 4399 EOF # violate MISRA_2004_Rule_10.5:Logical need here */
		}
		data >>=1u;
	}
	return reflection;
...
)
{
	...
	for (i = 0u; i < Crc_Length; i++)
	{
		...
		crc ^= Crc_DataPtr[i]; /* PRQA S 0492 EOF # violate MISRA_2004_Rule_17.4:No modification is needed here */
		...
		for (j = 0u; j < 0x8u; j++)
		{
			...
			if ((crc & 0x80u) > 0x0u)
			{
				crc <<= 1;	/* PRQA S 2911 EOF # violate MISRA_2004_Rule_21.1:Logical need here */
	...
	for (i = 0u; i < Crc_Length; i++)
	{
		j = crc ^ (uint8)Crc_DataPtr[i];
		crc = (uint8)Crc_8Table[j];
	}
...
)
{
	...
	for (i = 0u; i < Crc_Length; i++)
	{
		...
		crc ^= ((uint32)Crc_GetReflection_uint8(Crc_DataPtr[i]) << 0x18u);
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
    	j = (crc ^ (uint32)Crc_DataPtr[i]) & 0xFFu;
    	crc = Crc_32Table[j] ^ (crc >> 8u);
    }
	/* check if it is hardware method to calculate */
