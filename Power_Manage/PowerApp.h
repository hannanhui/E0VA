...
typedef enum
{
	...
	CAN_BUS_EVENT,				// CAN Bus event
	IGN_OFF_EVENT,				// LIN Bus event
	VALVE_PUMP_CTRL_EVENT		// Massage dormancy event of lumbar support
...
typedef enum
{
    VOLTAGE_TYPE_UP=0,
    VOLTAGE_TYPE_DOWN,
}VOLTAGE_UP_DOWN_TYPE;

typedef enum
{
    E_POWER_VOLTAGE_INDEX_0=0,
    E_POWER_VOLTAGE_INDEX_1,
    E_POWER_VOLTAGE_INDEX_2,
    E_POWER_VOLTAGE_INDEX_3,
    E_POWER_VOLTAGE_INDEX_MAX
} E_POWER_VOLTAGE_INDEX;
typedef enum
{
...
typedef struct
{
    unsigned char BattTimer;
    
}POWER_VOLTAGE_STATUS;
typedef struct 
{
    VOLTAGE_STATE VoltageMode;
    uint16_t SampleAD;
    uint16_t EnterDebounce;
    POWER_VOLTAGE_STATUS *Status;    
}POWER_VOLTAGE_TYPE_MANAGE;
typedef struct 
{
	  uint16_t minDebounceTime;
    uint16_t UpToOverHighAD;
    uint16_t UpToOverHighDebounce;   
		uint16_t UpToHighAD;
    uint16_t UpToHighDebounce; 
		uint16_t UpToNormalAD;
    uint16_t UpToNormalDebounce; 
		uint16_t UpToLowAD;
    uint16_t UpToLowDebounce; 
	  
		uint16_t DownToHighAD;
    uint16_t DownToHighDebounce; 
		uint16_t DownToNormalAD;
    uint16_t DownToNormalDebounce; 
		uint16_t DownToLowAD;
    uint16_t DownToLowDebounce; 
		uint16_t DownToUnderLowAD;
    uint16_t DownToUnderLowDebounce; 
}POWER_VOLTAGE_INIT_VALUE;
...
EXTERN_POWER_APP void PowerApp_SystemNoSleepEnSet(uint8_t opStatus);
EXTERN_POWER_APP uint8_t PowerApp_SystemNoSleepEnGet(void);
...
EXTERN_POWER_APP void PowerVoltage_Param_Init(POWER_VOLTAGE_INIT_VALUE *Config);
/*************************************************************************************************************************/
