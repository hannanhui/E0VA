...
const static uint8_t gs_aResponseEraseMemoryRoutineControlId[] = {0x71u, 0x01u, 0xDFu, 0xFFu};
/*erase memory routine cotnrol ID*/
const static uint8_t gs_aResponseCheckMemoryRoutineControlId[] = {0x71u, 0x01u, 0xDFu, 0xFEu};
...
BOOL TP_DriverWriteDataInTP(const uint32_t i_RxID, const uint32_t i_RxDataLen,  volatile uint8_t *i_pRxDataBuf)
{
	...
	CHECK_RESTRICTION(NULL != i_pRxDataBuf);
	CHECK_RESTRICTION(0u != i_RxDataLen);
...
static void Diag_Tx_Complete(void)
{
	...
	if(LIN_DIAG_SLAVER_RESPOND_ID == gs_stLinTPTxRxDataInfo.stLinTpDataInfo.xLinTpId)
	{
		if(gs_stLinTPTxRxDataInfo.stLinTpDataInfo.xPduDataLen <=  SF_DATA_MAX_LEN)
		{
			...
			pDestRoutineCltId = (uint8_t *)&gs_aResponseEraseMemoryRoutineControlId[0u];
        	FindCnt = sizeof(gs_aResponseEraseMemoryRoutineControlId);
			while(Index < FindCnt)
		    {
		        if(g_lin_Diag_Tx_data_buffer[Index+2] != pDestRoutineCltId[Index])
		        {
		        	checkRes = FALSE;
					break;
		        }
			...
			if(checkRes)
			{
				Uds_SetApp_Erase_Flag();
			}
			Index = 0u;
		    FindCnt = 0u;
		    pDestRoutineCltId = NULL;
			checkRes = TRUE;
			pDestRoutineCltId = (uint8_t *)&gs_aResponseCheckMemoryRoutineControlId[0u];
        	FindCnt = sizeof(gs_aResponseCheckMemoryRoutineControlId);
			while(Index < FindCnt)
		    {
		        if(g_lin_Diag_Tx_data_buffer[Index+2] != pDestRoutineCltId[Index])
		        {
...
void UDS_Tp_Init(void)
{
	...
	//l_ifc_tl_InstallCallback_Diag_Tx_Complete_LI0(LinIf_RxIndication(0,));
	//l_ifc_tl_InstallCallback_Diag_Rx_Complete_LI0(LinIf_TxConfirmation(0));
}
