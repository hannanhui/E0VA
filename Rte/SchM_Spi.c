 ...
 **                        Local Variables
 ******************************************************************************/
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_SpiCriticalLevel = 0U;
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_SpiCriticalCount = 0U;
/******************************************************************************
 **                        Macro  Definitions
 ******************************************************************************/
...
{
    if (kvInt_SpiCriticalCount > 0U)
    {
        /* Avoid QAC 2000 Warning */
    }
    else
    {
        kvInt_SpiCriticalLevel = Intr_LL_EnterCritical();
    }
    kvInt_SpiCriticalCount++;
}
...
void SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_00(void)
{
    ...
    if (kvInt_SpiCriticalCount == 0U)
    {
        Intr_LL_ExitCritical(kvInt_SpiCriticalLevel);
    }
    else
    {
        /* Avoid QAC 2000 Warning */
