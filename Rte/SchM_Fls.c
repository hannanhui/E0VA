 ...
 **                        Local Variables
 ******************************************************************************/
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_FlsCriticalLevel = 0U;
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_FlsCriticalCount = 0U;
/******************************************************************************
 **                        Macro  Definitions
 ******************************************************************************/
...
{
    if (kvInt_FlsCriticalCount > 0U)
    {
        /* Avoid QAC 2000 Warning */
    }
    else
    {
        kvInt_FlsCriticalLevel = Intr_LL_EnterCritical();
    }
    kvInt_FlsCriticalCount++;
}
...
void SchM_Exit_Fls_FLS_EXCLUSIVE_AREA_00(void)
{
    ...
    if (kvInt_FlsCriticalCount == 0U)
    {
        Intr_LL_ExitCritical(kvInt_FlsCriticalLevel);
    }
    else
    {
        /* Avoid QAC 2000 Warning */
