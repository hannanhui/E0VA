 ...
 **                        Local Variables
 ******************************************************************************/
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_WdgCriticalLevel = 0U;
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_WdgCriticalCount = 0U;
/******************************************************************************
 **                        Macro  Definitions
 ******************************************************************************/
...
{
    if (kvInt_WdgCriticalCount > 0U)
    {
        /* Avoid QAC 2000 Warning */
    }
    else
    {
        kvInt_WdgCriticalLevel = Intr_LL_EnterCritical();
    }
    kvInt_WdgCriticalCount++;
}
...
void SchM_Exit_Wdg_WDG_EXCLUSIVE_AREA_00(void)
{
    ...
    if (kvInt_WdgCriticalCount == 0U)
    {
        Intr_LL_ExitCritical(kvInt_WdgCriticalLevel);
    }
    else
    {
        /* Avoid QAC 2000 Warning */
