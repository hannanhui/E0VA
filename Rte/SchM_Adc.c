 ...
 **                        Local Variables
 ******************************************************************************/
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_AdcCriticalLevel = 0U;
static VOLATILE(uint32, Volatile, CAN_VAR) kvInt_AdcCriticalCount = 0U;
/******************************************************************************
 **                        Macro  Definitions
 ******************************************************************************/
...
{
    if (kvInt_AdcCriticalCount > 0U)
    {
        /* Avoid QAC 2000 Warning */
    }
    else
    {
        kvInt_AdcCriticalLevel = Intr_LL_EnterCritical();
    }
    kvInt_AdcCriticalCount++;
}
...
void SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_00(void)
{
    ...
    if (kvInt_AdcCriticalCount == 0U)
    {
        Intr_LL_ExitCritical(kvInt_AdcCriticalLevel);
    }
    else
    {
        /* Avoid QAC 2000 Warning */
