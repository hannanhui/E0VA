...
typedef enum
{
    ADC_VREF_EXTERNAL = 0U,                 /*!< External reference, Vref_H and Vref_L are selected 
                                                from external reference voltage source */
    ADC_VREF_INTERNAL                       /*!< Internal reference, tie Vref_H to VDDA, and tie 
                                                Vref_L to VSSA */
} ADC_VrefSource_t;
...
typedef enum
{
    ...
    ADC_P_CH25 = 25U,                       /*!< Internal channel, cmp dac out */
    ADC_P_CH28 = 28U,                       /*!< Internal channel, ldo_core, only for ADC0 */
    ADC_P_CH29 = 29U,                       /*!< Internal channel, ldo_flash, only for ADC0 */
    ADC_P_CH30 = 30U                        /*!< Internal channel, ldo_clk, only for ADC0 */
} ADC_P_Channel_t;

/**  
 *  @brief ADC Negative channel selection definition
 */
typedef enum
{
...
typedef enum
{
    ...
    ADC_COMPARE_MODE_WITHIN_CVL_CVH,        /*!< ADC compare within range [CVL,CVH] */
...
typedef enum
{
    ...
    ADC_TC_ERR_INT = 4U,                    /*!< ADC trigger conflict error interrupt*/
...
typedef enum
{
    ...
    ADC_STATUS_ACTIVE = 7U,                 /*!< ADC active status, 0: idle; 1: processing a 
                                                 conversion */
    ADC_STATUS_LOOP_IDLE,                   /*!< ADC loop mode state machine return to idle state */
    ADC_STATUS_TC_ERR_INT                   /*!< ADC trigger conflict error */
} ADC_Status_t;
...
typedef struct
{
    ...
    ADC_VrefSource_t adcVrefSource;      /*!< Configure ADC voltage reference source
                                             - ADC_VREF_EXTERNAL (0): External reference, Vref_H 
                                                and Vref_L are selected from external reference 
                                                voltage source
                                             - ADC_VREF_INTERNAL (1): Internal reference, tie 
                                                Vref_H to VDDA, and tie Vref_L to VSSA
                                          */
    ...
    ADC_Average_t adcAvgsSelect;         /*!< Configure ADC averages
                                             - ADC_AVGS_DISABLED (0): the number of ADC
                                                conversions which are averaged  is 1
                                             - ADC_AVGS_4 (1): the number of ADC conversions
                                                which are averaged  is 4
                                             - ADC_AVGS_8 (2): the number of ADC conversions
                                                which are averaged  is 8
                                             - ADC_AVGS_16(3): the number of ADC conversions
                                                which are averaged  is 16
                                             - ADC_AVGS_32(4): the number of ADC conversions
                                                which are averaged  is 32
                                          */
    uint32_t      adcSampleTime;         /*!< Set the time interval between two samples when average
                                                is enabled or in continues mode. Please refer to STS
                                                definition in RM to see the constraint */
    uint32_t      adcStableTime;            /*!< Configure ADC Stable time(clock cycles). Please
                                                refer to RM for more details */
} ADC_Config_t;
...
typedef struct
{
    ...
    ADC_N_Channel_t    adcChannelN;         /*!< Configure ADC negative channel
                                                - ADC_N_CH0 (0):   ADC Negative channel 0
                                                - ADC_N_CH1 (1):   ADC Negative channel 1
                                                - ADC_N_CH2 (2):   ADC Negative channel 2
                                                - ADC_N_VSS (3):   ADC Negative connected with VSS
                                             */
} ADC_ChannelConfig_t;
 ...
 * @return     none
 * 
 * @note       The adcConfigSet.adcSampleTime should be set in the range of 0 to 1023. The 
 *             adcConfigSet.adcStableTime should be set in the range of 0 to 2047. Please refer to
 *             the RM for more details.
 *
 */
 ...
 * @return     none
 *
 * @note       Disabling the ADC during conversion may cause the ADC to be stuck. Refer to the 
 *             errata document for more details.
 */
void ADC_Disable(ADC_ID_t adcId);

/**
 * @brief      Adc Channel Configure Function
 *
 * @param[in]  adcId:           Select the ADC ID:
 *                                - ADC0_ID
 *                                - ADC1_ID
...
/**
 * @brief      Adc TDG Trigger Configure Function
 *
 * @param[in]  adcId:            Select the ADC ID:
 *                                 - ADC0_ID
 *                                 - ADC1_ID
...
/**
 * @brief      Adc Compare Configure Function
 *
 * @param[in]  adcId:    Select the ADC ID:
 *                         - ADC0_ID
 *                         - ADC1_ID
 ...
 * @return     none
 * 
 * @note       The compareConfig->adcCvl and compareConfig->adcCvh should be set in the range of 0 
 *             to 4095.
 *
 */
 ...
 * @return     none
 *
 * @note      The fifoDepth should be set in the range of 1 to 16
 * 
 */
void ADC_FifoDepthRedefine(ADC_ID_t adcId, uint8_t fifoDepth);

/**
 * @brief      Adc FIFO Watermark Configure Function
 *
 * @param[in]  adcId:      Select the ADC ID:
 *                           - ADC0_ID
 *                           - ADC1_ID
 ...
 * @return     none
 * 
 * @note      The fwmark should be set in the range of 0 to 15
 *
 */
 ...
 * @return     none
 *
 * @note       After software reset, the result FIFO may be not empty. Refer to the errata document 
 *             for more details.
 */
 ...
 *                           - ADC_CMP_INT
 *                           - ADC_ERR_INT
 * @note    intType = ADC_INT_ALL, return value = SET, means all or anyone interrupt sets
 ...
 * @return     none
 *
 * @note      The stableTime should be set in the range of 0 to 2047
 * 
 */
 ...
 *                           -  ADC_CMP_INT
 *                           -  ADC_ERR_INT
 * @note    intType = ADC_INT_ALL is invalid
