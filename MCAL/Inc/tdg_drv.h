...
typedef enum
{
    TDG_CLEAR_MODULATOR = 0U,               /*!< Clear counter when counter
                                                equal to modulator */
    TDG_CLEAR_DELAY                         /*!< Clear counter when the channel with the maximum 
                                                accumulated delay output offset completes its delay 
                                                output. Note, the accumulated delay output offset 
                                                should not exceed 0xFFFF. Please refer to the 
...
typedef enum
{
    ...
    TDG_COUNT_INFINITY                      /*!< infinity loop mode */
}TDG_CountMode_t;
...
typedef struct
{
    ...
    TDG_ClkDivide_t clkDivide;              /*!< pre-scale clock source */
