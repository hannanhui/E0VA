...
#define TIM_RLDF_INT_MASK               (0x000002FFU)
#define TIM_TOF_INT_MASK                (0x000001FFU)
#define TIM_FAULT_INT_MASK              (0x0000000FU)
#define TIM_ALLCHANNEL_INT_MASK         (0x000000FFU)
#define TIM_EDGEPWM_MASK             (0x8U)
#define TIM_FLTSR_WPEN_MASK             (0x7FU)
#define TIM_GLBSR_WPDIS_MASK            (0x7FFU)
#define TIM_CMCN_ELS_MASK               (0xFCU)
#define TIM_CMCN_CHF_MASK               (0x020U)
#define TIM_CMCN_CMS_MASK               (0xF3U)
#define TIM_CMCN_ICRST_MASK             (0x7FU)
#define TIM_CMCN_DMA_MASK               (0xBFU)
#define TIM_CMCN_CHIE_MASK              (0xEFU)

#define TIM_CMCN_CMS_SHIFT              (0x2U)
#define TIM_CMCN_DMA_SHIFT              (0x6U)
#define TIM_CMCN_CHIE_SHIFT             (0x4U)
#define TIM_CMCN_ICRST_SHIFT            (0x7U)
...
static void TIM_ChIntHandler(TIM_ID_t timId)
{
    ...
    intStatus = TIMw->TIM_GLBSR  & TIM_ALLCHANNEL_INT_MASK;
...
static void TIM_FaultIntHandler(TIM_ID_t timId)
{
    ...
    if (TIMx->TIM_FLTSR.FAULTF == 1U)
    {
        /* clear interrupt status */
        TIMw->TIM_FLTSR = (uint32_t)TIM_FAULT_INT_MASK;
    }
...
static void TIM_OverflowIntHandler(TIM_ID_t timId)
{
    tim_reg_t * TIMx = timRegPtr[timId];
    tim_reg_w_t * TIMw = timRegWPtr[timId];
    if (TIMx->TIM_GLBSR.TOF == 1U)
    {
        /* clear interrupt status */
        TIMw->TIM_GLBSR = (uint32_t)TIM_TOF_INT_MASK;
    }
...
static void TIM_RlflIntHandler(TIM_ID_t timId)
{
    tim_reg_t * TIMx = timRegPtr[timId];
    tim_reg_w_t * TIMw = timRegWPtr[timId];
    if (TIMx->TIM_GLBSR.RLDF == 1U)
    {
        /* clear interrupt status */
        TIMw->TIM_GLBSR = TIM_RLDF_INT_MASK;
    }
...
void TIM_WriteProtectionEnable(TIM_ID_t timId)
{
    ...
    TIMw->TIM_FLTSR= TIM_FLTSR_WPEN_MASK;
}
...
void TIM_WriteProtectionDisable(TIM_ID_t timId)
{
    ...
    TIMw->TIM_GLBSR =  TIM_GLBSR_WPDIS_MASK;
}
 ...
 * @return none
 *
 * @note The counter will start to run after CKSRC is set to none zero, so the 'divider' will delay
 *  a few cycles to effect. You can call TIM_StartCounter to set CKSRC and PSDIV first ,then call
 *  TIM_StopCounter to stop counter. Finally call TIM_StartCounter to start counting to avoid this 
 *  influence.
 */
                            ...
                            const TIM_OutputSWCtrlConfig_t* config)
{
    ...
    TIMw->TIM_CMCn[channelId] =((TIMw->TIM_CMCn[channelId]&TIM_CMCN_ELS_MASK)|0x1U|(TIM_CMCN_CHF_MASK)) ;
    TIM_SetOutputPolarity(timId, channelId, config->polarity);
    TIM_SetCombineCmd(timId, (TIM_PairId_t)pairId, config->complementaryEnable); 
...
void TIM_InputCaptureInit(TIM_ID_t timId, const TIM_InputCaptureConfig_t * config)
{
    ...
    for(i = 0; i < config->numChannels; i++)
    {
        ...
        TIM_SetDualEdgeCaptureCmd(timId, (TIM_PairId_t)pairId, DISABLE);
        TIMw->TIM_CMCn[channelId] =((((TIMw->TIM_CMCn[channelId]&TIM_CMCN_CMS_MASK)&TIM_CMCN_ELS_MASK)
                                |((uint32_t)config->inputChConfig[i].edgeConfig))|TIM_CMCN_CHF_MASK);
        TIMw->TIM_CMCn[channelId] = ((TIMw->TIM_CMCn[channelId]&TIM_CMCN_ICRST_MASK)
        |(((uint32_t)config->inputChConfig[i].resetEnable)<<TIM_CMCN_ICRST_SHIFT)|TIM_CMCN_CHF_MASK);
...
void TIM_DualEdgeCaptureInit(TIM_ID_t timId, const TIM_DualEdgeCaptureConfig_t * config)
{
    ...
    for(i = 0; i < config->numPairs; i++)
    {
        ...
        TIM_SetCombineCmd(timId, pairId, DISABLE);
        TIMw->TIM_CMCn[mainChannelId] = (((TIMw->TIM_CMCn[mainChannelId]&TIM_CMCN_CMS_MASK)
        |(uint32_t)((uint32_t)config->config[i].captureMode<<TIM_CMCN_CMS_SHIFT))|TIM_CMCN_CHF_MASK);
        TIMw->TIM_CMCn[mainChannelId] = (((TIMw->TIM_CMCn[mainChannelId]&TIM_CMCN_ELS_MASK)
        |((uint32_t)config->config[i].mainEdgeConfig))|TIM_CMCN_CHF_MASK);
        TIMw->TIM_CMCn[secondChannelId] = (((TIMw->TIM_CMCn[secondChannelId]&TIM_CMCN_ELS_MASK)
        |((uint32_t)config->config[i].secondEdgeConfig))|TIM_CMCN_CHF_MASK);
...
void TIM_OutputCompareInit(TIM_ID_t timId, const TIM_OutputCompareConfig_t * config)
{
    ...
    for(i = 0; i < config->numChannels; i++)
    {
        ...
        TIM_SetCombineCmd(timId, (TIM_PairId_t)pairId, DISABLE);
        TIM_SetDualEdgeCaptureCmd(timId, (TIM_PairId_t)pairId, DISABLE);
        TIMw->TIM_CMCn[channelId] = ((TIMw->TIM_CMCn[channelId]&TIM_CMCN_CMS_MASK)|
        (uint32_t)((uint32_t)0x1U<<TIM_CMCN_CMS_SHIFT)|TIM_CMCN_CHF_MASK);
        TIMw->TIM_CMCn[channelId] = ((TIMw->TIM_CMCn[channelId]&TIM_CMCN_ELS_MASK)|
                        ((uint32_t)config->outputConfig[i].outputMode)|TIM_CMCN_CHF_MASK);
...
void TIM_OutputCenterAlignedPwmConfig(TIM_ID_t timId, const TIM_PwmConfig_t * config)
{
    ...
    for(i = 0; i < config->numChannels; i++)
    {
        ...
        TIM_SetDualEdgeCaptureCmd(timId, (TIM_PairId_t)pairId, DISABLE);
        TIMw->TIM_CMCn[channelId]= ((TIMw->TIM_CMCn[channelId]& TIM_CMCN_ELS_MASK)
                    |(uint32_t)config->pwmChConfig[i].pwmModeConfig|TIM_CMCN_CHF_MASK);
...
void TIM_OutputEdgeAlignedPwmConfig(TIM_ID_t timId, const TIM_PwmConfig_t * config)
{
    ...
    for(i = 0; i < config->numChannels; i++)
    {
        ...
        TIM_SetDualEdgeCaptureCmd(timId, (TIM_PairId_t)pairId, DISABLE);
        TIMw->TIM_CMCn[channelId]= ((TIMw->TIM_CMCn[channelId]&TIM_CMCN_CMS_MASK)|
                                (uint32_t)TIM_EDGEPWM_MASK|TIM_CMCN_CHF_MASK);
        TIMw->TIM_CMCn[channelId]= ((TIMw->TIM_CMCn[channelId]&TIM_CMCN_ELS_MASK)|
                                (uint32_t)config->pwmChConfig[i].pwmModeConfig|TIM_CMCN_CHF_MASK);            
        TIM_SetOutputPolarity(timId, channelId, config->pwmChConfig[i].polarity);
        TIMx->TIM_CCVn[channelId].CCV = config->pwmChConfig[i].compareValue;
...
void TIM_OutputComplementaryPwmConfig(TIM_ID_t timId, const TIM_CompPwmConfig_t * config)
{
    ...
    for(i = 0; i < config->numPairs; i++)
    {
        ...
        TIMw->TIM_CMCn[mainChannelId] = ((TIMw->TIM_CMCn[mainChannelId]&TIM_CMCN_ELS_MASK)
                    |(uint32_t)config->cPwmChConfig[i].pwmModeConfig|TIM_CMCN_CHF_MASK);
...
void TIM_DMACtrl(TIM_ID_t timId, TIM_ChannelId_t channelId, ControlState_t cmd)
{
    ...
    TIMw->TIM_CMCn[channelId]=(uint32_t)((TIMw->TIM_CMCn[channelId] &TIM_CMCN_DMA_MASK)|
                      (uint32_t)((uint32_t)cmd <<TIM_CMCN_DMA_SHIFT)|TIM_CMCN_CHF_MASK) ;
}
...
void TIM_IntMask(TIM_ID_t timId, TIM_INT_t intType, IntMask_t intMask)
{
    ...
    switch (intType)
    {
        ...
        case TIM_INT_CH6:
        case TIM_INT_CH7:
            TIMw->TIM_CMCn[intType] = (uint32_t)((TIMw->TIM_CMCn[intType] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            break;
        case TIM_INT_RLD:
        ...
        case TIM_INT_ALL:
            TIMw->TIM_CMCn[0] = (uint32_t)((TIMw->TIM_CMCn[0] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[1] = (uint32_t)((TIMw->TIM_CMCn[1] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[2]= (uint32_t)((TIMw->TIM_CMCn[2] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[3] = (uint32_t)((TIMw->TIM_CMCn[3] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[4] = (uint32_t)((TIMw->TIM_CMCn[4] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[5] = (uint32_t)((TIMw->TIM_CMCn[5] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[6] = (uint32_t)((TIMw->TIM_CMCn[6] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMw->TIM_CMCn[7] = (uint32_t)((TIMw->TIM_CMCn[7] & TIM_CMCN_CHIE_MASK)|
            (uint32_t)((uint32_t)cmd <<TIM_CMCN_CHIE_SHIFT)|TIM_CMCN_CHF_MASK);
            TIMx->TIM_GLBCR.RLDIE = (uint32_t)cmd;
            TIMx->TIM_GLBCR.TOIE = (uint32_t)cmd;
            TIMx->TIM_FLTCR.FLTIE = (uint32_t)cmd;
...
void TIM_IntClear(TIM_ID_t timId, TIM_INT_t intType)
{
    ...
    switch (intType)
    {
        ...
        case TIM_INT_RLD:
            if (TIMx->TIM_GLBSR.RLDF == 1U)
            {
                TIMw->TIM_GLBSR = (uint32_t)TIM_RLDF_INT_MASK;
            }
            break;
 ...
 *
 * @return intstatus
 * @note   TIM_INT_ALL is invalid as input .
 */
...
void TIM_FaultStatusClear(TIM_ID_t timId, TIM_FaultChannelId_t channelId)
{
    ...
    {
        case TIM_FAULT_CHANNEL_0:
            TIMw->TIM_FLTSR = 0x1EU;
        break;
        
        case TIM_FAULT_CHANNEL_1:
            TIMw->TIM_FLTSR = 0x1DU;
            break;
