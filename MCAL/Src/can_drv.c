...
#define CAN_FDCTRL_TDCOFF_MASK  ((uint32_t)0x1FU << CAN_FDCTRL_TDCOFF_SHIFT)
#define CAN_FDCTRL_TDCFAIL_MASK ((uint32_t)1U << 14U)
#define CAN_FDCTRL_TDCEN_MASK   ((uint32_t)1U << 15U)
#define CAN_FDCTRL_FDRATE_MASK  ((uint32_t)1U << 31U)
#define CAN_FDCTRL_MBDSR0_SHIFT (16U)
#define CAN_FDCTRL_MBDSR0_MASK  ((uint32_t)3U << CAN_FDCTRL_MBDSR0_SHIFT)
#define CAN_FDCTRL_MBDSR1_SHIFT (19U)
#define CAN_FDCTRL_MBDSR1_MASK  ((uint32_t)3U << CAN_FDCTRL_MBDSR1_SHIFT)
#define CAN_FDCTRL_MBDSR2_SHIFT (22U)
#define CAN_FDCTRL_MBDSR2_MASK  ((uint32_t)3U << CAN_FDCTRL_MBDSR2_SHIFT)
#define CAN_FDCTRL_MBDSR3_SHIFT (25U)
#define CAN_FDCTRL_MBDSR3_MASK  ((uint32_t)3U << CAN_FDCTRL_MBDSR3_SHIFT)
 ...
 *
 * @param[in]  dlcValue: the DLC field value
 *
 * @return payload size
 *
 */
...
/**
 * @brief  Computes the DLC field value, and the actual data size given a data 
 *         length (in bytes).
 *
 * @param[in]  dataLen:datalength
 * @param[in-out]  dlc: length of data in bytes
 * @param[in-out]  dataSize:payload size
 *
 * @return none
 *
 */
    ...
    else
    {
        ...
        else
        {
            if(CANx->CAN_MCR.FDEN != 0U)
            {
                ...
                {
                    regVal = CANxw->CAN_FDCTRL;
                    regVal &= (~CAN_FDCTRL_TDCFAIL_MASK);
                    regVal |= CAN_FDCTRL_FDRATE_MASK;
                    CANxw->CAN_FDCTRL = regVal;
                }
            }
...
static void CAN_ClearRam(CAN_Id_t id)
{
    ...
    uint32_t ramSize;
    uint32_t rxImrSize;
    volatile uint32_t *RAM = &(CANxw->CAN_MB[0].CS);
...
static void CAN_BusOffHandler(CAN_Id_t id)
{
    ...
    if((status & CAN_INT_MSK_FLAG_BUS_OFF) != 0U)
    {
        ...
        else
        {
            ...
            {
                /* user can add code here to handle this error */
            }
        }            
    }
 ...
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  status: the status to be get. CAN_STATUS_ALL is invalid.
 *
 * @return   status
 *           - SET
 *           - RESET
 ...
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  config: points to the configuration struct. null pointer is not allowed.
 *
 * @note       For CAN 0-5, 64 message buffers are available.
 *             For CAN 6-7, 128 message buffers are available.
 *
...
ResultStatus_t CAN_Init(CAN_Id_t id, const CAN_Config_t *config)
{
    ...
    if(SUCC == retVal)
    {
        ...
        CANxw->CAN_FDCTRL &= (uint32_t)(~(CAN_FDCTRL_TDCFAIL_MASK | CAN_FDCTRL_TDCEN_MASK));
        ...
        if(ENABLE == config->fdEn )
        {
            ...
            regVal = CANxw->CAN_FDCTRL;
            regVal &=
                (~(CAN_FDCTRL_TDCFAIL_MASK | CAN_FDCTRL_MBDSR0_MASK | CAN_FDCTRL_MBDSR1_MASK));
            regVal |= (((uint32_t)(config->payload0) << CAN_FDCTRL_MBDSR0_SHIFT) &
                CAN_FDCTRL_MBDSR0_MASK);
            regVal |= (((uint32_t)(config->payload1) << CAN_FDCTRL_MBDSR1_SHIFT) &
                CAN_FDCTRL_MBDSR1_MASK);
            if((CAN_ID_6 == id ) || (CAN_ID_7 == id ))
            {
                regVal &=
                    (~(CAN_FDCTRL_TDCFAIL_MASK | CAN_FDCTRL_MBDSR2_MASK | CAN_FDCTRL_MBDSR3_MASK));
                regVal |= (((uint32_t)(config->payload2) << CAN_FDCTRL_MBDSR2_SHIFT) &
                    CAN_FDCTRL_MBDSR2_MASK);
                regVal |= (((uint32_t)(config->payload3) << CAN_FDCTRL_MBDSR3_SHIFT) &
                    CAN_FDCTRL_MBDSR3_MASK);
            }
            CANxw->CAN_FDCTRL = regVal;
 ...
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  timeSeg: A pointer to the CAN bit timing settings. null pointer is not allowed.
 *
 * @return      status
 *             - SUCC -- successful
 *             - ERR -- some error
 ...
 * @param[in]  mask:Mask Value. Its format depends on the RX FIFO id filter 
   ...
   RTR   | IDE   | IDmask0(std:29-19, ext:29-16)| RTR    | IDE    | IDmask1
                                                            (std:13-3, ext:13-0)
   In the extended frame format B, IDmask0/IDmask1 corresponds to the 14 most 
 ...
 *                    For CAN6-7, the maximum mbIdx is 127 if payload size is
 *                    8-byte.
 * @param[in]  txInfo: TX MB info. null pointer is not allowed.
 * @param[in]  msgId: message ID. 11-bit standard id or 29-bit extended id
 ...
 * @param[in]  txInfo: TX MB info. null pointer is not allowed.
 * @param[in]  msgId: message ID. 11-bit standard id or 29-bit extended id
 * @param[in]  msgData:  data of the message. null pointer is not allowed.
 * @param[in]  localPrio: the local priority field, if the TX priority mode is
 *                        CAN_TX_PRI_LOCAL_PRI_EN, this parameter indicates the
 *                        local priority
 *
 ...
 * @param[in]  msgId: message ID. 11-bit standard id or 29-bit extended id
 * @param[in]  msgData: data of the message. null pointer is not allowed.
 * @param[in]  localPrio: the local priority field for the MB. valid range is [0, 7]
 *
 * @return     status
 *            - SUCC
 *            - ERR: mbIdx is invalid
 ...
 * @param[in]  idFilterTable: points to the ID filter table. null pointer is not allowed.
 *
 * @note 
                              ...
                              const CAN_IdFilter_t *idFilterTable)
{
    ...
    volatile uint32_t *filterTable = &(CANxw->CAN_MB[6].CS);
    
    filterNum = ((uint32_t)CANx->CAN_CTRL2.RFFN + 1U) << 3U;
 ...
 *            stored. null pointer is not allowed.
 *
 * @return     status
 *
 *
...
void CAN_ReadRxFifo(CAN_Id_t id, CAN_MsgBuf_t *msgBuff)
{
    ...
    uint8_t payloadSize;
    /*PRQA S 0303 ++*/
    volatile CAN_Mb_t *mbAddr = (CAN_Mb_t *)(uint32_t)&(CANxw->CAN_MB[0].CS);
    /*PRQA S 0303 --*/
    volatile uint8_t  *mbData;
 ...
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  offset: Transceiver Delay Compensation Offset. valid range is [0, 31].
 *
 * @return    status
 *            - SUCC -- successful
 *            - ERR -- some error
...
ResultStatus_t CAN_FdTdcEnable(CAN_Id_t id, uint8_t offset)
{
    ...
    {
        regVal = CANxw->CAN_FDCTRL;
        regVal &= (uint32_t)(~(CAN_FDCTRL_TDCFAIL_MASK | CAN_FDCTRL_TDCOFF_MASK));
        regVal |= CAN_FDCTRL_TDCEN_MASK;
        regVal |= (((uint32_t)offset << CAN_FDCTRL_TDCOFF_SHIFT) & CAN_FDCTRL_TDCOFF_MASK);
        CANxw->CAN_FDCTRL = regVal;
 ...
 *                    enabled, otherwise, it will return ERR.
 *                    For CAN0-5, the maximum mbIdx is 63 if payload size is
 *                    8-byte. 
 *                    For CAN6-7, the maximum mbIdx is 127 if payload size is
 *                    8-byte.
 ...
 *                   pointer. null pointer is not allowed.
 *
 * @return     status
 *
 *
 ...
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  config: match wakeup config. null pointer is not allowed.
 *
 * @return  status
 *          - SUCC -- successful
 *          - ERR -- some error
 ...
 *                       structure. null pointer is not allowed.
 *
 * @return  status
 *          - SUCC -- successful
 *          - ERR -- some error
 ...
 * @param[in]  msgBuff: point to the address where the message buffer will be
 *                      stored. The size is 8 at least. null pointer is not allowed.
 *
 * @return     status
 *
 *
...
/**
 * @brief      Install callback function for interrupts except message buffer 
 *             interrupts. 
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  intType: select interrupt type. CAN_INT_MB and CAN_INT_ALL are invalid.
...
{
    ResultStatus_t retVal = SUCC;
#if (CAN_INSTANCE_NUM == 4)
    if((uint32_t)id >= (uint32_t)CAN_ID_4)
#else
    if((uint32_t)id >= (uint32_t)CAN_ID_6)
#endif
    {
 ...
 *                     (MB64-MB95) to be configured. Each bit corresponds a MB
 *                     (bit0 - MB64, bit1 - MB65, ... ,bit31 - MB95). If a bit 
 *                     is 1,it means the corresponding MB to be configured.
 *                     For other interrupt type, this parameter can be ignored.
 * @param[in]  mb96t127: It is only valid for CAN6 and CAN7.
 *                      if intType is CAN_INT_MB, this parameter selects the MBs
 *                     (MB96-MB127) to be configured. Each bit corresponds a MB
 ...
 * @param[in]  id: select the CAN ID
 * @param[in]  mbIdx: the MB index
 *                    For CAN0-5, the maximum mbIdx is 63.
 *                    For CAN6-7, the maximum mbIdx is 127.
 * @param[in]  intMask: MASK/UNMASK
 *
 * @return     ResultStatus_t
 *             - SUCC
 ...
 *                     corresponding MB is unmasked(enabled); if a bit is 0, the 
 *                     corresponding MB is masked(disabled).
 ...
 *                     (MB0-MB31) to be configured. Each bit corresponds a MB
 *                     (bit0 - MB0, bit1 - MB1, ... ,bit31 - MB31). If a bit 
 *                     is 1, it means the corresponding MB interrupt is to be 
 *                     cleared.
 *                     For other interrupt type, this parameter can be ignored.
 * @param[in]  mb32t63: if intType is CAN_INT_MB, this parameter selects the MBs
 *                     (MB32-MB63) to be configured. Each bit corresponds a MB
...
/**
 * @brief      Clear CAN status, only CAN_STATUS_ERROVR take effect
 *
 * @param[in]  id: select the CAN ID
 * @param[in]  status: the status to clear
 *              - CAN_STATUS_ERROVR: error overrun status
 *              - others: have no effect since other statuses are managed by hardware.
 *
 * @return   none
 *
 */ 
