                                                   ...
                                                   const FLASH_CmdConfig_t *config)
{
    ...
    else
    {
        ...
        {
            //stat = Eep_ExecuteCommandInt(FLASH_CMD_PGMPHR);
        }
    }
...
ResultStatus_t Eep_Program(uint32_t flashAddr, uint32_t len, const uint8_t *dataP,
                             flash_cb_t callBack)
{
    uint32_t count;
    uint32_t bufAddr;
    ...
    else
    {
        ...
        {
            /*PRQA S 0306 ++*/
            stat = FLASH_ProgramDflashPhrase((flashAddr + count), (uint8_t *)(bufAddr + count),
                                       &config);
            /*PRQA S 0306 --*/
            if(SUCC != stat)
            {
...
ResultStatus_t FLASH_EraseDflashSector(uint32_t addr, const FLASH_CmdConfig_t *config)
{
    ...
    else
    {
        ...
        {
            //stat = Eep_ExecuteCommandInt(FLASH_CMD_ERSSCR);
        }
    }
...
uint8_t DFlash_Write(uint32_t flashAddr, uint32_t len, const uint8_t *dataP)
{
	ResultStatus_t stat = ERR;
	uint32_t Len16=0;
	uint8_t data[16]={0};
...
uint8_t DFlash_EraseNSectors(uint32_t addr,uint32_t sectors)
{
	ResultStatus_t stat = ERR;
