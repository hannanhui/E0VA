...
ResultStatus_t WDOG_Enable(void)
{
    ...
    WDOG_UNLOCK_CONFIG();
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~WDOG_CS_INTF_MASK;
    uTmp |= WDOG_CS_WDOGE_MASK;
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_ConfigAllowControl(ControlState_t allow)
{
    ...
    WDOG_UNLOCK_CONFIG();
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_CFGUA_MASK;
    uTmp |= WDOG_CS_CFGUA_SET((uint32_t)allow);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_WindowModeControl(ControlState_t winEnable)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_WINE_MASK;
    uTmp |= WDOG_CS_WINE_SET((uint32_t)winEnable);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_SetClockSource(WDOG_ClkSource_t clkSource)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_CLKS_MASK;
    uTmp |= WDOG_CS_CLKS_SET((uint32_t)clkSource);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_StopModeControl(ControlState_t cmd)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_STOPE_MASK;
    uTmp |= WDOG_CS_STOPE_SET((uint32_t)cmd);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_WaitModeControl(ControlState_t cmd)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_WAITE_MASK;
    uTmp |= WDOG_CS_WAITE_SET((uint32_t)cmd);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_DebugModeControl(ControlState_t cmd)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_DEBUGE_MASK;
    uTmp |= WDOG_CS_DEBUGE_SET((uint32_t)cmd);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
...
ResultStatus_t WDOG_SetTestMode(WDOG_TestMode_t testMode)
{
    ...
    uTmp = wDOGxw->WDOG_CS;
    uTmp &= ~(uint32_t)WDOG_CS_INTF_MASK;
    uTmp &= ~(uint32_t)WDOG_CS_TSTM_MASK;
    uTmp |= WDOG_CS_TSTM_SET((uint32_t)testMode);
    wDOGxw->WDOG_CS = uTmp;
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
 ...
 *
 */
ResultStatus_t WDOG_ClearIntStatus(void)
{
    wdog_reg_w_t * wDOGxw = (wdog_reg_w_t *)(wdogRegWPtr);
    uint32_t primask;
    ASMV_KEYWORD( "MRS %0, PRIMASK " : "=r"(primask));
 ...
 *
 * @note       intType=WDOG_INT_ALL is invalid.
 * 
 * @return    none
 *
 */
...
void WDOG_DriverIRQHandler(void)
{
    ...
    WDOG_UNLOCK_CONFIG();
    wdogRegWPtr->WDOG_CS |= wdogIntFlagMask[WDOG_INT_BEFORERESET];
    if (0U == primask)
    {
        COMMON_ENABLE_INTERRUPTS();
    }
