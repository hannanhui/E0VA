...
#define UART_INTSTA_IID_PTIME         0x02U
...
#define UART_LSI_OE        ((uint32_t)1U << 1U)
#define UART_LSI_PE        ((uint32_t)1U << 2U)
#define UART_LSI_FE        ((uint32_t)1U << 3U)
#define UART_LSI_BI        ((uint32_t)1U << 4U)
#define UART_LSI_RFE       ((uint32_t)1U << 7U)
#define UART_LSI_ADDR_RCVD ((uint32_t)1U << 8U)
...
static const uint32_t uartInterruptStatusTable[]=
{
    ...
    UART_INTSTA_IID_PTIME,          /*!< UART_INT_PTIME */  
...
static void UART_IntHandler(UART_ID_t uartId)
{
    ...
    switch(intId)
    {
        ...
        case UART_INTSTA_IID_TBEI:
            /* PTIME interrupt is enable*/
            if(0x82U == (UARTxw->UART_DLH_IER.UART_IER & (uint32_t)UART_CHECK_PTIME))
            {
                if(uartIsrCb[uartId][UART_INT_PTIME] != NULL)
                {
                    /* call the callback function */
                    uartIsrCb[uartId][UART_INT_PTIME]();
                }
                /* Disable the interrupt if callback function is not setup */
                else
                {
                    UARTxw->UART_DLH_IER.UART_IER &= ~((uint32_t)UART_CHECK_PTIME);
                }
            }
 ...
 * @param[in]  uartId: Select the UART port,should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  intType: select the interrupt from UART_Int_t, except UART_INT_ALL.
 * @param[in]  cbFun: pointer to callback function.
 * 
 * @note       UART_INT_ALL shall not be selected.
 *
 * @return     none
 *
 */
 ...
 * @param[in]  uartId:  Select the UART port,should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  uartConfig: Pointer to a UART configuration structure, null pointer is not allowed.
 *
 * @return     none
 *
 */
 ...
 * @param[in]  uartId:  UART peripheral selected, should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  len:     the length of break, its range is 0xB-0x1F.
 *
 * @return     
 *             - SUCC : break is sent
 *             - ERR : break length is invalid
 * 
 * @note:      Due to internal clock synchronization, the actual output break length
 *             will be shorter than the set length, with a maximum difference of no
 *             more than 1 Tbit.
 */
 ...
 * @param[in]  uartId: Select the UART port, should be UART0_ID, UART1_ID,
 *                     UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  addr:   The 8-bit receive address.
 * @return     none
 *
 */
 ...
 * @param[in]  uartId:  Select the UART port,should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  addr:  The 8-bit transmit address.
 *
 * @return     none
 *
 */
 ...
 * @param[in]  uartId:  Select the UART port,should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  data:  The 9-bit data to be sent.
 * @return     none
 *
 */
 ...
 * @param[in]  uartId:  Select the UART port, should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  fifoConfig:  Pointer to a FIFO configuration, null pointer is not allowed.
 * @return     none
 *
 */
 ...
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 *
 * @param[in]  config: config idle detect, null pointer is not allowed.
 *
 * @return     none
 *
 */
 ...
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  config:  points to the configuration of LIN mode, null pointer is not allowed.
 * @return     none
 */
 ...
 * @param[in]  uartId:  Select the UART port.It should be  UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  header:  points to the header to be send, null pointer is not allowed.
 * @return     status
 *             - SUCC : header is sent
                                           ...
                                           const UART_LinHeader_t *header)
{
    ...
    else
    {
        ...
        UARTx->UART_LIN_ID_VALUE.ID = header->id;    
...
uint32_t UART_LinGetId(UART_ID_t uartId)
{
    ...
    return UARTx->UART_LIN_ID_VALUE.ID;
}
 ...
 * @param[in]  uartId:  Select the UART port.It should be  UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  rsp:  Points to reponse structure to be sent, null pointer is not allowed.
 * @return
 ...
 * @param[in]  len:  response length, its range is 0x1-0x8.
 * @return
 ...
 * @param[in]  uartId:  Select the UART port.It should be  UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  len:  response length to be read, its range is 0x1-0x8.
 ...
 * @param[in]  uartId:  Select the UART port, should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  intType: select one interrupt from UART_Int_t, except UART_INT_ALL and 
 *                      UART_INT_ASYNC.
 * @note       1. If only need async status, can use UART_GetLineStatus function.
 *             2. UART_INT_ALL and UART_INT_ASYNC shall not be selected.
 * 
 ...
 * @param[in]  uartId:  Select the UART port, should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  intType:  Specifies the interrupt type from UART_Int_t.
 * @param[in]  intMask:  Enable/Disable Specified interrupt type
 *
 * @return     none
 * 
 * @note       UART_INT_LSI is not suggested to be unmasked in lin slave mode. When
 *             lin slave receives header, the break\sync field\PID will be put 
 *             into uart receive buffer register, it will cause OE, BI and FE flags set.
 *             If UART_INT_LSI is unmasked, line status interrupt will be triggered.
 *
 */
 ...
 *                            is ignored. Only bit0-5 is valid. Its range is 0 - 0x3F.
 * @return     none
 *
 */
 ...
 *                   otherwise is disabled. Its range is 0 - 0x1FFFF.
 * @return    none
 *
 */
 ...
 * @param[in]  uartId:  Select the UART port.It should be UART0_ID, UART1_ID,
 *                      UART2_ID, UART3_ID, UART4_ID, UART5_ID.
 * @param[in]  idNum:  the number of IDs to be configured. The range is 0-16, if idNum is 0, 
 *                     it means that no ID filter values will be configured.
 * @param[in]  idValues:  a pointer to the ID filter value that will be used to
 *                        config the ID filters. The size of the memory must
 *                        not smaller than idNum, null pointer is not allowed.
 * @return
 *            - SUCC
 *            - ERR : parameter error
 *
