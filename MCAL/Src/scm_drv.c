 ...
 * @return     none
 * 
 * @note       Users should clear the FPU exception flags manually in their callback functions, 
 *             if not, FPU exception will happen again until user clear the FPU exception flags.
 *             The method to clear FPU exception flags is to clear the exception status bits in FPU
 *             register FPSCR, as well as the stacked FPSCR before exception return. 
 *             For more information, please refer Arm v7-M Architecture Reference Manual.
 *
 *             The specific interrupt will be disabled if the corresponding interrupt callback is 
...
static void SCM_IntHandler(void)
{
    ...
    /* Only clear cache bit error flag, the FPU exception flags(FPU_INEXACT_FLAG,FPU_OVERFLOW_FLAG,
       FPU_UNDERFLOW_FLAG, FPU_INVALIDOP_FLAG, FPU_DIVZERO_FLAG,FPU_DENORMALIN_FLAG) in register 
       SCM_MISCSTAT1 will not be cleared */
 ...
 *             - SCM_CCACHE_BE_INT
 *             - SCM_FPU_INEXACT_INT
 *             - SCM_FPU_OVERFLOW_INT
 *             - SCM_FPU_UNDERFLOW_INT
 *             - SCM_FPU_INVALIDOP_INT
  *            - SCM_FPU_DIVZERO_INT
 *             - SCM_FPU_DENORMALIN_INT
 *             - SCM_INT_ALL
 ...
 *
 * @param[in]  intType: interrupt type id. SCM_INT_ALL is invalid.
 *             - SCM_CCACHE_BE_INT
 *             - SCM_FPU_INEXACT_INT
 *             - SCM_FPU_OVERFLOW_INT
 *             - SCM_FPU_UNDERFLOW_INT
 ...
 *             - SCM_FPU_DIVZERO_INT
 *             - SCM_FPU_DENORMALIN_INT
 * @param[in]  cbFun  : callback function pointer. This parameter cannot be a null pointer.
 *
 * @return     None
 *
 */
...
void SCM_SwTrig(SCM_SwTrig_t swTrigType)
{
    ...
    {
    case SCM_SWTRIG0:
        scmRegPtr->SCM_MISCCTL1.SWTRIG0 = 1U;
        break;
    case SCM_SWTRIG1:
        scmRegPtr->SCM_MISCCTL1.SWTRIG1 = 1U;
        break;
    case SCM_SWTRIG2:
        scmRegPtr->SCM_MISCCTL1.SWTRIG2 = 1U;
        break;
    case SCM_SWTRIG3:
        scmRegPtr->SCM_MISCCTL1.SWTRIG3 = 1U;
        break;
...
/**
 * @brief      Clear Software Trigger To TMU
 *
 * @param[in]  swTrigType: software trigger type
                        - SCM_SWTRIG0
                        - SCM_SWTRIG1
 ...
 *
 */
void SCM_ClearSwTrig(SCM_SwTrig_t swTrigType)
{
    switch (swTrigType)
    {
    case SCM_SWTRIG0:
        scmRegPtr->SCM_MISCCTL1.SWTRIG0 = 0U;
        break;
    case SCM_SWTRIG1:
        scmRegPtr->SCM_MISCCTL1.SWTRIG1 = 0U;
        break;
    case SCM_SWTRIG2:
        scmRegPtr->SCM_MISCCTL1.SWTRIG2 = 0U;
        break;
    case SCM_SWTRIG3:
        scmRegPtr->SCM_MISCCTL1.SWTRIG3 = 0U;
        break;
 ...
 * @return     None
 *
 * @note       If cache is enabled, cache parity fault and cache parity miss will be enabled.
 *             If cache is disabled, cache parity fault and cache parity miss will be disabled.
 * 
 */
void SCM_CacheEnable(ControlState_t cacheCtrl)
{
    if (DISABLE == cacheCtrl)
    {
        ...
        scmRegPtr->SCM_MISCCTL1.CCACHE_PARITY_FAULT_EN = (uint32_t)DISABLE;
        scmRegPtr->SCM_MISCCTL1.CCACHE_PARITY_MISS_EN = (uint32_t)DISABLE;
    }
    else
    {
        scmRegPtr->SCM_MISCCTL1.CCACHE_PARITY_FAULT_EN = (uint32_t)ENABLE;
        scmRegPtr->SCM_MISCCTL1.CCACHE_PARITY_MISS_EN = (uint32_t)ENABLE;
        scmRegPtr->SCM_MISCCTL1.CACHE_DIS = 0U;
    }
}
 ...
 *             It is recommended to enable this feature when cache is enabled
 *
