...
#define SERU_CH_FLAG_MASK       (0xFFFFFEFFU)
#define SERU_CH_EN_SHIFT        ((uint32_t)0x0U)
#define SERU_CH_INT_CFG_SHIFT   ((uint32_t)0x1U)
#define SERU_CH_INT_TYP_SHIFT   ((uint32_t)0x2U)
#define SERU_CH_EOUT_CFG_SHIFT  ((uint32_t)0x3U)
#define SERU_CH_RESET_CFG_SHIFT ((uint32_t)0x4U)
#define SERU_CH_RESET_TYP_SHIFT ((uint32_t)0x5U)
#define SERU_CH_RESET_DLY_SHIFT ((uint32_t)0x6U)
...
static void SERU_WriteCfgCHnRegWithUnmaskChFlag(SERU_ChannelNO_t chNo, uint32_t bitShift,
                                                    uint32_t newValue, uint32_t bitWidth);
...
/**
 * @brief      SERU write configuration value
 *
 * @param[in]  chNo: channel number
 * @param[in]  bitShift: field bit to be shifted
 * @param[in]  newValue: new value to be written
 * @param[in]  bitWidth: the width of this bit field, range of [1:32]
 *
 * @return     none
 *
 */
...
{   
    uint32_t configVal;
    configVal 
        = ((seruRegWPtr->SERU_CFG_CHn[(uint32_t)chNo]) & ~(((1UL << bitWidth) - 1UL) << bitShift));
    configVal |= (newValue << bitShift);
    configVal &= SERU_CH_FLAG_MASK;
    seruRegWPtr->SERU_CFG_CHn[(uint32_t)chNo] = configVal;
}
 ...
 *
 * @param[out]  seruConfig: A pointer to the SERU default configuration structure.
 *
 * @return     none
 *
 */
 ...
 * @return     none
 *
 * @note       If user expects a reset when OSC40M loss of clock is detected
 *             using channel 8, SERU_CFG_CH8.CH_RESET_TYP should be set to
 *             SERU_SEL_RST_COLD.
 *             If user expects a normal or cold reset when a low-power mode
 *             is detected using channel 4, SERU_CFG_CHn4[CH_INT_CFG]
 *             should be set to 0.
 *             If user expects interrupts when a low-power mode is detected
 *             using channel 4, SERU_CFG_CH4[CH_RESET_CFG] should be set to 0.
...
void SERU_Init(const SERU_ChannelConfig_t *channelConfig)
{
    ...
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_EOUT_CFG_SHIFT,
                                                    (uint32_t)(channelConfig->eoutEnable), 1U);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_INT_TYP_SHIFT,
                                                    (uint32_t)(channelConfig->intType), 1U);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_INT_CFG_SHIFT,
                                                    (uint32_t)(channelConfig->intEnable), 1U);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_RESET_TYP_SHIFT,
                                                    (uint32_t)(channelConfig->resetType), 1U);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_RESET_DLY_SHIFT,
                                                    (uint32_t)(channelConfig->resetDelay), 2U);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_RESET_CFG_SHIFT,
                                                    (uint32_t)(channelConfig->resetEnable), 1U);
    seruRegPtr->SERU_CSR.EOUT_PS= (uint32_t)(channelConfig->polaritySel);

    if(DISABLE == channelConfig->FFIEnable)
    {
        seruRegWPtr->SERU_CH_FFI &= ~(((uint32_t)1U << (uint32_t)channelConfig->channelNo));
    }
    else
    {
        seruRegWPtr->SERU_CH_FFI |= ((uint32_t)1U <<(uint32_t)channelConfig->channelNo);
    }
    SERU_WriteCfgCHnRegWithUnmaskChFlag(channelConfig->channelNo, SERU_CH_EN_SHIFT,
                                                    (uint32_t)(channelConfig->channelEnable), 1U);
 ...
 *               - RESET: No configuration register parity mismatch event occurred.
 *               - SET: Configuration register parity mismatch event occurred.
 *
 */
 ...
 *
 * @param[in]  chNo : channel number.
 *                    - SERU_CHANNEL_0: Channel 0 is selected
...
{
    SERU_EnterMode(SERU_CONFIG_MODE);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(chNo, SERU_CH_EN_SHIFT,(uint32_t)chCtrl, 1U);
    SERU_EnterMode(SERU_NORMAL_MODE);
}
 ...
 * @return     None
 *
 * @note       If user expects a normal or cold reset when a low-power mode
 *             is detected using channel 4, SERU_CFG_CHn4[CH_INT_CFG]
 *             should be set to 0.
 *             If user expects interrupts when a low-power mode is detected
 *             using channel 4, SERU_CFG_CH4[CH_RESET_CFG] should be set to 0.
...
{
    SERU_EnterMode(SERU_CONFIG_MODE);
    SERU_WriteCfgCHnRegWithUnmaskChFlag(chNo, SERU_CH_INT_CFG_SHIFT,(uint32_t)chCtrl, 1U);
    SERU_EnterMode(SERU_NORMAL_MODE);
}
